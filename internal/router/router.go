...
func (r *Router) Pick(ctx context.Context, members []pool.Member, sessionID string, trySticky bool) (*pool.Member, error) {
	if len(members)==0 { return nil, ErrNoMember }
	...
	if len(enabled)==0 { return nil, ErrNoHealthy }
	// filter healthy (cooldown)
	ids := make([]string, len(enabled))
	for i,m:=range enabled { ids[i]=m.ID }
	healthyIDs := ids
	if r.store != nil {
		healthyIDs = r.store.FilterHealthy(ctx, ids)
	}
	if len(healthyIDs)==0 { return nil, ErrNoHealthy }
	...
	for _,id:=range healthyIDs { set[id]=true }
	for _,m:=range enabled { if set[m.ID] { healthy = append(healthy, m)} }
	...
	if trySticky && sessionID!="" && r.store!=nil {
		if pinned, ok := r.store.GetSticky(ctx, sessionID); ok {
			for i := range healthy {
				if healthy[i].ID==pinned { return &healthy[i], nil }
			}
		}
	}
	...
	if sessionID!="" {
		...
		total := 0
		for _,m:=range healthy { total+=m.Weight }
		if total==0 { total=len(healthy) }
		pick := int(h) % total
		acc:=0
		for i:=range healthy {
			w:=healthy[i].Weight
			if w==0 { w=1 }
			acc+=w
			if pick < acc { return &healthy[i], nil }
		}
