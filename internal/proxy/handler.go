...
func NewHandler(loader interface{ Active() *pool.Pool }, r *router.Router, s *store.Store, stickyMode string) *Handler {
	return &Handler{
		loader: loader,
		router: r,
		store: s,
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if sessionID=="" { sessionID = req.Header.Get("x-session-id") }
...
func (h *Handler) forward(ctx context.Context, orig *http.Request, body []byte, m pool.Member) (int, http.Header, []byte, error) {
	...
	if orig.URL.RawQuery != "" { target += "?" + orig.URL.RawQuery }
	...
	for k, vals := range ah {
		for _, v := range vals {
			if k=="anthropic-beta" {
				...
				} else if existing=="" { req.Header.Set(k, v)}
			} else { req.Header.Set(k, v)}
		}
	}
	if len(rwBody)>0 { req.Header.Set("Content-Length", itoa(len(rwBody))) }
...
func (h *Handler) cooldown(m *pool.Member, ttlSec int) {
	if h.store==nil || m==nil { return }
	if ttlSec<=0 { ttlSec = m.CooldownSec }
...
func filterNotTried(members []pool.Member, tried map[string]bool) []pool.Member {
	var out []pool.Member
	for _, m := range members { if !tried[m.ID] { out=append(out,m)} }
	return out
}
...
func recordTokens(memberID string, body []byte) {
	...
	if err:=json.Unmarshal(body,&env); err!=nil||env.Usage==nil { return }
